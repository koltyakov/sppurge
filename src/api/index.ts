...
export class Delete {
  ...
  public deleteFile(context: IContext, filePath: string): Promise<any> {
    const restUrl = `${context.siteUrl}/_api/Web/GetFileByServerRelativeUrl(@FilePath)` +
  ...
  public deleteFolder(context: IContext, folderPath: string): Promise<any> {
    const restUrl = `${context.siteUrl}/_api/Web/GetFolderByServerRelativeUrl(@FolderPath)` +
